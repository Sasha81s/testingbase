  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { ok: false, chain, perps_volume_24h: null, error: { url: baseUrl, status: res.status } },
        { status: 500 }
      )
    }
      ...
      return NextResponse.json(
        {
          ...
          error: { url: baseUrl, msg: 'missing 24h key', keys: Object.keys(json || {}) },
        },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json(
      {
        ...
        chain,
        perps_volume_24h: null,
        error: { url: baseUrl, msg: e?.message || String(e) },
      },
      { status: 500 }
    )
  }
